import { STORES, OUTBOX_LOGS, META_KEYS, logEvent, nowMs, randomId, bytesOfString, requestIdle, shouldDebugClientLogs } from "./schema.js";
import { getByKey, withStore } from "./openDB.js";
const STUDY_SCHEDULE_PREFIX = "study-schedule-";
const outboxWakeHooks = new WeakMap();
const enqueueMarks = [];
const STALE_SENDING_TIMEOUT_MS = 20000;
const DEFAULT_SENDER_TIMEOUT_MS = 15000;
function canonicalizeScheduleKey(key) {
    return key.startsWith(STUDY_SCHEDULE_PREFIX) ? STUDY_SCHEDULE_PREFIX : key;
}
function inferCoalesceGroup(input) {
    if (input.coalesceGroup)
        return input.coalesceGroup;
    if (input.key.startsWith(STUDY_SCHEDULE_PREFIX))
        return `${input.userId || "anon"}:${STUDY_SCHEDULE_PREFIX}`;
    return undefined;
}
function inferDedupeKey(input) {
    if (input.dedupeKey)
...
export async function isMigrationInProgress(db) {
    const rec = await getByKey(db, STORES.meta, META_KEYS.migrating);
    return Boolean(rec?.value);
}
export async function enqueueOutboxEvent(db, input) {
    const dedupeKey = inferDedupeKey(input);
    let result;
    await withStore(db, STORES.outbox, "readwrite", async (store) => {
        const all = await new Promise((resolve, reject) => {
            const req = store.getAll();
            req.onsuccess = () => resolve((req.result || []));
            req.onerror = () => reject(req.error || new Error("IDB getAll outbox failed"));
        });
        const existing = all.find((evt) => (evt.status === "queued" || evt.status === "failed") &&
    ...
    while (enqueueMarks.length && ts - enqueueMarks[0] > 1000)
        enqueueMarks.shift();
...
export function notifyOutboxHasWork(db) {
...
export function createDebouncedOutboxEnqueuer(db, delayMs = 2500, scheduleCoalesceMs = 1200) {
    const timers = new Map();
    const latest = new Map();
    const lastScheduleFlushByGroup = new Map();
    return {
        enqueue(input) {
            const dedupeKey = inferDedupeKey(input);
            ...
            const t = window.setTimeout(async () => {
                ...
                if (group)
                    lastScheduleFlushByGroup.set(group, nowMs());
...
function computeBackoffMs(retryCount, base, max, multiplier = 1) {
...
async function withTimeoutReject(promise, timeoutMs, makeError) {
...
async function listReadyEvents(db, maxEvents, maxBytes, maxPayloadBytes) {
    const now = nowMs();
    const all = await withStore(db, STORES.outbox, "readonly", async (store) => {
        const req = store.getAll();
        return new Promise((resolve, reject) => {
            req.onsuccess = () => resolve((req.result || []));
    ...
    for (const evt of sorted) {
        ...
        if (batch.length >= maxEvents || (batch.length > 0 && bytes + size > maxBytes))
            break;
        batch.push(evt);
        bytes += size;
    }
...
async function updateEvents(db, events, mutator) {
    if (!events.length)
        return;
    await withStore(db, STORES.outbox, "readwrite", async (store) => {
        for (const evt of events)
            store.put(mutator(evt));
    });
}
async function markSending(db, events) {
    const ts = nowMs();
    await updateEvents(db, events, (evt) => ({ ...evt, status: "sending", lastAttemptAt: ts }));
}
async function recoverStaleSendingEvents(db, timeoutMs = STALE_SENDING_TIMEOUT_MS) {
    const ts = nowMs();
    const stale = await withStore(db, STORES.outbox, "readonly", async (store) => {
        const req = store.getAll();
        return new Promise((resolve, reject) => {
            req.onsuccess = () => resolve((req.result || []).filter((evt) => evt.status === "sending" && typeof evt.lastAttemptAt === "number" && ts - evt.lastAttemptAt > timeoutMs));
...
async function markFailedWithBackoff(db, events, error, baseBackoffMs, maxBackoffMs, maxRetryCount) {
    if (!events.length)
        return;
    const ts = nowMs();
...
async function markDead(db, events, reason, error) {
    if (!events.length)
        return;
    const ts = nowMs();
    await updateEvents(db, events, (evt) => ({
...
async function markSpecificFailed(db, eventsById, failures, baseBackoffMs, maxBackoffMs, maxRetryCount) {
    if (!failures.length)
        return;
    ...
    for (const failure of failures) {
        ...
        if (failure.permanent)
            permanentEvents.push(evt);
        else
            retryEvents.push(evt);
    }
    if (permanentEvents.length) {
        await markDead(db, permanentEvents, permanentEvents.map((e) => reasonById.get(e.id)).find(Boolean) || "permanent_error", new Error("Partial permanent send failure"));
    }
...
function createSlidingCounter(windowMs) {
    const marks = [];
    return {
        push(ts = nowMs()) {
            marks.push(ts);
            while (marks.length && ts - marks[0] > windowMs)
                marks.shift();
        },
        count(ts = nowMs()) {
            while (marks.length && ts - marks[0] > windowMs)
...
export function createOutboxProcessor(db, options) {
    ...
    const baseBackoffMs = Math.min(Math.max(options.baseBackoffMs ?? 2000, 500), 60000);
    const maxBackoffMs = Math.min(Math.max(options.maxBackoffMs ?? 120000, baseBackoffMs), 600000);
    ...
    const senderTimeoutMs = Math.min(Math.max(options.senderTimeoutMs ?? DEFAULT_SENDER_TIMEOUT_MS, 2000), 120000);
    const idleScheduling = options.idleScheduling ?? true;
    let stopped = true;
    let loopTimer = null;
    let inFlight = false;
    const sentEventsCounter = createSlidingCounter(60000);
    const sentBatchesCounter = createSlidingCounter(60000);
        ...
        try {
            ...
            try {
                const sendResult = await withTimeoutReject(options.sender(batch), senderTimeoutMs, () => {
                    const err = new Error(`outbox_sender_timeout:${senderTimeoutMs}ms`);
                    err.status = 408;
                    return err;
                });
                if (sendResult && typeof sendResult === "object") {
                    ...
                    const sentEvents = sentIds.map((id) => batchById.get(id)).filter(Boolean);
                    if (sentEvents.length)
                        await markDone(db, sentEvents);
                    if (failed.length) {
                    ...
                    for (let i = 0; i < sentCountNow; i += 1)
                        sentEventsCounter.push(nowMs());
                    if (sentCountNow > 0 || failed.length > 0)
                ...
                if (error?.permanent) {
                    await markDead(db, batch, error?.reason || "permanent_error", error);
    ...
    return {
        ...
        stop() {
            ...
            if (outboxWakeHooks.get(db))
                outboxWakeHooks.delete(db);
        },
        poke() {
            if (!stopped)
                schedule(0);
...
export async function getOutboxStatusSummary(db) {
    const all = await withStore(db, STORES.outbox, "readonly", async (store) => {
        const req = store.getAll();
        return new Promise((resolve, reject) => {
            req.onsuccess = () => resolve((req.result || []));
    ...
    for (const evt of all) {
        if (evt.status === "queued")
            queued += 1;
        else if (evt.status === "sending")
            sending += 1;
        else if (evt.status === "failed")
            failed += 1;
        else if (evt.status === "dead")
            dead += 1;
    }
    return {
        total: all.length,
...
export async function pruneDeadOutboxEvents(db, options = {}) {
    const olderThanMs = Math.max(0, Number(options.olderThanMs ?? 12 * 60 * 60 * 1000));
    const reasonSet = new Set((Array.isArray(options.reasons) ? options.reasons : ["schema_incompatible_id"])
        .map((reason) => String(reason || "").trim())
        .filter(Boolean));
    const cutoff = nowMs() - olderThanMs;
    const removable = await withStore(db, STORES.outbox, "readonly", async (store) => {
        const req = store.getAll();
        return new Promise((resolve, reject) => {
            req.onsuccess = () => resolve(((req.result || [])).filter((evt) => {
                if (evt.status !== "dead")
                    return false;
                const payload = evt.payload && typeof evt.payload === "object" ? evt.payload : null;
                const deadReason = String(payload?.deadReason || "");
                if (deadReason && reasonSet.has(deadReason))
                    return true;
...
export async function verifyOutboxSafety(db, options = {}) {
    const notes = [];
    const before = await withStore(db, STORES.outbox, "readonly", async (store) => {
        const req = store.getAll();
        return new Promise((resolve, reject) => {
            req.onsuccess = () => resolve((req.result || []));
    ...
    if (options.migrationProbe) {
        ...
        const afterProbe = await withStore(db, STORES.outbox, "readonly", async (store) => {
            const req = store.getAll();
            return new Promise((resolve, reject) => {
                req.onsuccess = () => resolve((req.result || []));
                req.onerror = () => reject(req.error || new Error("verify getAll failed"));
        ...
        if (!migrationIsolationObserved)
            notes.push("Migracao alterou outbox (nao deveria gerar eventos).");
    }
    ...
    const after = await withStore(db, STORES.outbox, "readonly", async (store) => {
        const req = store.getAll();
        return new Promise((resolve, reject) => {
            req.onsuccess = () => resolve((req.result || []));
            req.onerror = () => reject(req.error || new Error("verify getAll failed"));
    ...
    if (!dedupWorks)
        notes.push("Dedup by dedupeKey falhou em verifyOutboxSafety");
    if (!coalesceWorks)
        notes.push("Coalesce de study-schedule-* falhou em verifyOutboxSafety");
    if (!batchLimitObserved)
        notes.push(`Batch limit nao observado (largestBatch=${largestBatch}).`);
    if (!rateLimitObserved)
        notes.push(`Rate limit nao observado (sentCount=${sentCount}).`);
