import { META_KEYS, MIGRATION_LOGS, MIGRATION_VERSION, SCHEMA_VERSION, STORES, bytesOfString, logEvent, nowMs, requestIdle, sha256Hex, shouldMigrateKey, yieldToMainThread, randomId, } from "./schema.js";
...
const CHECKSUM_PENDING_WAIT_TIMEOUT_MS = 20000;
...
function getLogger(options) {
    return (event, payload) => {
        if (options?.logger)
            return options.logger(event, payload);
        logEvent(event, payload);
    };
}
async function setMetaBulk(db, entries) {
...
async function getMetaValue(db, key) {
    const rec = await getByKey(db, STORES.meta, key);
    return rec?.value;
}
async function getMigrationMetaState(db) {
    ...
    return {
        ...
        migratedAt: map[META_KEYS.migratedAt] ?? null,
        migrateStats: map[META_KEYS.migrateStats] ?? null,
        sourceSnapshot: map[META_KEYS.sourceSnapshot] ?? null,
        targetSnapshot: map[META_KEYS.targetSnapshot] ?? null,
        migrationResult: map[META_KEYS.migrationResult] ?? null,
        migrationLock: map[META_KEYS.migrationLock] ?? null,
        auditWarnings: map[META_KEYS.auditWarnings] ?? null,
        skippedLargeKeys: map[META_KEYS.skippedLargeKeys] ?? null,
        checksumPendingCount: map[META_KEYS.checksumPendingCount] ?? null,
        shadowAuditExpiresAt: map[META_KEYS.shadowAuditExpiresAt] ?? null,
    };
}
function listSelectedLocalStorageKeys(storage) {
    const keys = [];
...
async function getMirrorRecord(db, key) {
    return getByKey(db, STORES.localStorageMirror, key);
}
async function upsertMirrorRecord(db, record) {
    await withStore(db, STORES.localStorageMirror, "readwrite", async (store) => {
        store.put(record);
    });
}
async function getMirrorRecordsByKeys(db, keys) {
    const map = new Map();
    if (!keys.length)
        return map;
    await withStore(db, STORES.localStorageMirror, "readonly", async (store) => {
...
async function processKeysInChunks(keys, options, runner) {
    let bytesInChunk = 0;
    let keysInChunk = 0;
...
async function checksumValue(value) {
    return sha256Hex(value);
}
async function computeCurrentLocalEntry(storage, key) {
...
async function estimateStoragePressure(storage, options) {
...
async function acquireMigrationLock(db, ownerId) {
    return withStore(db, STORES.meta, "readwrite", async (store) => {
        const now = nowMs();
        ...
        const lock = { ownerId, acquiredAt: now, expiresAt: now + LOCK_TTL_MS };
        store.put({ key: META_KEYS.migrationLock, value: lock, updatedAt: now });
        return { acquired: true, lock };
    });
}
async function refreshMigrationLock(db, ownerId) {
...
function startLockHeartbeat(db, ownerId) {
    let timer = null;
    let stopped = false;
    const tick = async () => {
        if (stopped)
...
async function finalizePendingChecksums(db, keys, storage, options, logger) {
    const started = nowMs();
    while (true) {
...
async function passOneCopy(db, storage, ctx, options, logger) {
    ...
    await processKeysInChunks(ctx.keys, options, async (key) => {
        ...
        try {
            ...
            if (isQuotaExceededError(error))
                throw new Error("INSUFFICIENT_STORAGE:QuotaExceededError");
            throw error;
        }
        if (entry.checksumPending)
            pendingCount += 1;
        ctx.migratedKeys += 1;
        chunkMigrated += 1;
...
async function secondPassRefreshChangedKeys(db, storage, ctx, options, logger, warnings) {
    let processed = 0;
    await processKeysInChunks(ctx.keys, options, async (key) => {
        processed += 1;
        ...
        if (prevChecksum && prevChecksum === latestComparableChecksum)
            return latest.size;
...
async function buildSourceSnapshot(ctx) {
    const keys = [...ctx.keys];
    let totalBytes = 0;
    for (const key of keys)
        totalBytes += ctx.sourceSizes[key] || 0;
    return { keys, checksumsByKey: { ...ctx.sourceChecksums }, totalBytes };
}
async function buildTargetSnapshotAndValidate(db, sourceSnapshot, options, logger) {
    const checksumsByKey = {};
    let totalBytes = 0;
    const mismatches = [];
    let processed = 0;
    let pendingCount = 0;
...
async function maybeRunShadowAudit(db, storage, sourceKeys, options, warnings) {
    if (options.disableShadowAudit)
        return;
    const now = nowMs();
...
export async function verifyMigrationIntegrity(db) {
    const meta = await getMigrationMetaState(db);
    ...
    for (const key of keys) {
        ...
        if (checksum !== rec.checksum)
            checksumMismatches.push(`checksum:${key}`);
    }
    return {
...
export async function migrateLocalStorageMirror(db, options = {}) {
    ...
    try {
        ...
        try {
            ...
            const { targetSnapshot, mismatches, pendingCount } = await buildTargetSnapshotAndValidate(db, sourceSnapshot, { chunkKeys, maxBytesPerChunk, onBeforeProcessKey: options.onBeforeProcessKey }, logger);
            if (!sameSnapshotKeys(sourceSnapshot, targetSnapshot))
                mismatches.push("Contagem/lista de chaves divergente entre source e target.");
            if (pendingCount > 0)
                mismatches.push(`Ainda existem ${pendingCount} checksums pendentes.`);
...
export async function getMigrationMeta(db) {
    return getMigrationMetaState(db);
}
//# sourceMappingURL=migrateLocalStorageMirror.js.map