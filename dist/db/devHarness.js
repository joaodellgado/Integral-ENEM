...
import { STORES, META_KEYS, nowMs, shouldMigrateKey, sha256Hex } from "./schema.js";
...
function bigString(sizeKb, seed = "x") {
    return new Array(sizeKb * 1024 + 1).join(seed).slice(0, sizeKb * 1024);
}
function seedFakeLocalStorage() {
    const keys = [
...
async function validateMirrorIntegrity(db) {
    const mirror = await getAllFromStore(db, STORES.localStorageMirror);
...
async function getMirrorByKey(db, key) {
    const records = await getAllFromStore(db, STORES.localStorageMirror);
    return records.find((r) => r.key === key);
}
export async function runLocalStorageMirrorMigrationHarness() {
    ...
    const localBackup = new Map(managedKeys.map((k) => [k, localStorage.getItem(k)]));
    ...
    try {
        ...
        if (!db)
            throw new Error("Harness DB unavailable");
        ...
        const outboxAfterFacadeSet = await getAllFromStore(db, STORES.outbox);
        ...
        const lowSpace = await migrateLocalStorageMirror(db, {
            storageEstimateOverride: { usage: 999000, quota: 1000000 },
            chunkKeys: 5,
        });
        ...
        const hugeRec = (await getAllFromStore(db, STORES.localStorageMirror)).find((r) => r.key === hugeKey);
        ...
        await clearIDBStores(db);
        seedFakeLocalStorage();
        const outboxBeforeMigration = await getAllFromStore(db, STORES.outbox);
        ...
        const oversizedAfterMark = (await getAllFromStore(db, STORES.outbox))[0];
        ...
        assert(oversizedAfterMark.payload.oversizedDisposition === "metadata_only", "oversized marker should drop raw payload");
        await withStore(db, STORES.outbox, "readwrite", async (store) => {
...
//# sourceMappingURL=devHarness.js.map